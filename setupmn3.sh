...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='otscoin.conf'
CONFIGFOLDER='/root/.otscoin'
COIN_DAEMON='otscoind'
COIN_CLI='otscoin-cli'
COIN_PATH='/usr/local/bin/'
COIN_REPO=''
COIN_TGZ='https://github.com/otscoinproject/otscoin/releases/download/1.0/otscoin.zip'
COIN_ZIP=$(echo $COIN_TGZ | awk -F'/' '{print $NF}')
COIN_NAME='OtsCoin'
COIN_PORT=5730 #Updated Port
RPC_PORT=6783
...
function download_node() {
  ...
  wget -q $COIN_TGZ
  sudo chmod 775 otscoin.zip
  apt install unzip 
  compile_error
#   tar xvzf $COIN_ZIP -C $COIN_PATH >/dev/null 2>&1
unzip otscoin.zip 
cd otscoin
sudo chmod 755 otscoind
sudo chmod 755 otscoin-cli
cp * /usr/local/bin
chmod -R 775 *
cd ..
  cd - >/dev/null 2>&1
...
echo "OtsCoin MN installer Depends Starting"
