...
#if defined(NDEBUG)
#error "OtsCoin cannot be compiled without assertions."
#endif
...
static unsigned int nStakeMinAgeV1 = 2 * 60 * 60; // 2 hours
static unsigned int nStakeMinAgeV2 = nStakeMinAgeV1; //2 hr
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zOtsCoin spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zOtsCoin spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zOtsCoin spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    //POS Starts here
bool phase2 = nHeight > Params().LAST_POW_BLOCK() && nHeight <= 18000;
bool phase3 = nHeight >= 18001 && nHeight <= 21000;
bool phase4 = nHeight >= 21001 && nHeight <= 28800;
bool phase5 = nHeight >= 28801 && nHeight <= 35000;
bool phase6 = nHeight >= 35001 && nHeight <= 70000;
bool phase7 = nHeight >= 70001 && nHeight <= 90000;
bool phase8 = nHeight >= 90001 && nHeight <= 120000;
bool phase9 = nHeight >= 120001 && nHeight <= 180000;
bool phase10 = nHeight >= 180001 && nHeight <= 350000;
bool phase11 = nHeight >= 350001 && nHeight <= 650000;


		
	int64_t nSubsidy = 1 * COIN;
		if (nHeight == 1) return 700000 * COIN;
	if(phase1) 
        nSubsidy = 2* COIN;
    else if (phase2)
        nSubsidy = 10* COIN;
...
int64_t GetTreasuryAward(int nHeight)
{
	if(IsTreasuryBlock(nHeight)) {
		if(nHeight == nStartTreasuryBlock)
			return 100 * COIN; //200 for the first treasury block, 10 - reward to PoS
		else{
            // int p = 3/100;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from OtsCoin
...
{
    RenameThread("otscoin-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZOTSMinted()
{
...
void RecalculateZOTSSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zOTS supply
...
bool RecalculateOTSSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zOtsCoin with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zOtsCoinSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert OTS to zOTS
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zOtsCoin spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zOtsCoin serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zOtsCoin mints and %d zOtsCoin spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // OtsCoin: We use certain sporks during IBD, so check to see if they are
