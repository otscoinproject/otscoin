...
#include "zotscoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zOTS ought to be enough for anybody." - Bill Gates, 2017
    ui->zOTSpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzOTSSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzOTS->setEnabled(false);
        ui->pushButtonMintzOTS->setToolTip(tr("zOTS is currently disabled due to maintenance."));

        ui->pushButtonSpendzOTS->setEnabled(false);
        ui->pushButtonSpendzOTS->setToolTip(tr("zOTS is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zOTSpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzOTS_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zOTS is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zOTS...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zOTS in ") +
...
void PrivacyDialog::on_pushButtonSpendzOTS_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zOTS is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zOTS
        sendzOTS();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zOTS
    sendzOTS();
}

void PrivacyDialog::on_pushButtonZOtsCoinControl_clicked()
{
    ZOtsCoinControlDialog* zOtsCoinControl = new ZOtsCoinControlDialog(this);
    zOtsCoinControl->setModel(walletModel);
    zOtsCoinControl->exec();
}

void PrivacyDialog::setZOtsCoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzOtsCoinSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzOTS()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid OtsCoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zOTSpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zOTS is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " OTS </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zOTS</b>";
    ...
    // use mints from zOtsCoin selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZOtsCoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZOtsCoinControlDialog::GetSelectedMints();
    }
    ...
    // Clear zotscoin selector in case it was used
    ZOtsCoinControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zOtsCoin Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " OtsCoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zOtsCoin Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zOTS </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zOTS "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zOTS "));
    ui->labelzOTSAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzOTSSyncStatus->setVisible(fShow);
}
