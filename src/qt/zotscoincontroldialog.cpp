...
#include "ui_zotscoincontroldialog.h"
...
std::list<std::string> ZOtsCoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZOtsCoinControlDialog::listMints;

ZOtsCoinControlDialog::ZOtsCoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZOtsCoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZOtsCoinControlDialog::~ZOtsCoinControlDialog()
{
    delete ui;
}

void ZOtsCoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZOtsCoinControlDialog::updateList()
{
...
void ZOtsCoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZOtsCoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZOtsCoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZOtsCoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZOtsCoinControlDialog::GetSelectedMints()
{
...
void ZOtsCoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
